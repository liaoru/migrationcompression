...
class restore{
    private:
		...
		unsigned char data[536870912 + 65536 * 4];
    ...
    public:
		...
		void doit() //类入口，返回聚合完毕的数据
		{
			...
			int *s;
			int num = MAXLEN / LEN;
			while( (rc = fread(data,sizeof(unsigned char), (536870912 + 65536 * 4),infile)) != 0 )
    		{
    		    if(rc < (536870912 + 65536 * sizeof(int)))
                {
                ...
                else
                {
                    ...
                    for(int i = 0; i < 65536; i++)
                    {
                        fwrite(data + p[i] * (8192 + 4) + 4, sizeof( unsigned char ), 8192, outfile );
                    }
                }
